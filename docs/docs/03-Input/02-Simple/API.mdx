...
A Model is a simpleinput editor for handling simpleinput token-based inputs
without any special parsing required.
...
```
CursorOffset returns the visual offset of the cursor in terms of number of
terminal cells. Use this for calculating visual dimensions such as input
width/height.
...
```
Init is part of the editor interface. It does not need to be invoked by end
users.
...
```
OnUpdateStart is part of the editor interface. It does not need to be invoked by
end users.
...
```
Runes returns the raw text entered by the user as a list of runes. This is
useful for indexing and length checks because doing these operations on strings
does not work well with some unicode characters.
...
```
TokenValues returns the tokenized input text. This **does not** include
delimiter tokens.
...
```
View is part of the editor interface. It does not need to be invoked by end
users.
