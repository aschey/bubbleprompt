...
func (m *LexerModel[T]) createWhitespaceToken(start int, end int, index int) editor.Token {
	token := editor.Token{
...
func (m *LexerModel[T]) unstyledView(text []rune, showCursor bool) string {
	viewBuilder := editor.NewViewBuilder(m.CursorIndex(), lipgloss.NewStyle(), " ", showCursor)
...
func (m *LexerModel[T]) setSelectedToken(token *editor.Token) {
...
func (m *LexerModel[T]) currentToken(runes []rune, tokenPos int) editor.Token {
	if len(m.tokens) == 0 {
		return editor.Token{Index: -1}
	}
	if len(m.Value()) == 0 {
		return m.tokens[0]
	}
...
func (m *LexerModel[T]) TokensBeforeCursor() []editor.Token {
	...
	for _, token := range m.tokens {
		if token.End() <= cursor {
			...
			tokens = append(tokens, editor.Token{
...
func (m *LexerModel[T]) OnSuggestionChanged(suggestion editor.Suggestion[T]) {
	...
	if m.IsDelimiterToken(token) {
		if m.CursorIndex() < len(runes) {
			...
			if m.IsDelimiterToken(token) {
				token = editor.Token{Start: token.End()}
			}
		} else {
			token = editor.Token{
				Start: token.End(),
...
func (m *LexerModel[T]) currentTokenBeforeCursor(token editor.Token) []rune {
	start := token.Start
	cursor := m.CursorIndex()
	if start > cursor {
		return []rune("")
