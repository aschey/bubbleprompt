...
func (p *ParticipleLexer) Lex(inputStr string) ([]editor.Token, error) {
	...
	tokens := make([]editor.Token, len(lexerTokens))
	for i, token := range lexerTokens {
		tokens[i] = editor.TokenFromPos(token.Value, symbols[token.Type], i, token.Pos)
	}
