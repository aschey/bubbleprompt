...
type cmdViewBuilder[T CmdMetadataAccessor] struct {
	model            Model[T]
	viewBuilder      *editor.ViewBuilder
	showPlaceholders bool
	showCursor       bool
}

func newCmdViewBuilder[T CmdMetadataAccessor](model Model[T], viewMode editor.ViewMode) cmdViewBuilder[T] {
	showCursor := !model.textinput.Blink()
	if viewMode == editor.Static {
		showCursor = false
	}
	showPlaceholders := viewMode == editor.Interactive
	viewBuilder := editor.NewViewBuilder(model.CursorIndex(), model.formatters.Cursor, model.defaultDelimiter, showCursor)
