...
type Model[T CmdMetadataAccessor] struct {
	...
	argIndex            int
	currentFlag         *editor.Suggestion[T]
...
func (m *Model[T]) FlagSuggestions(inputStr string, flags []FlagInput, suggestionFunc func(FlagInput) T) []editor.Suggestion[T] {
	...
	for _, flag := range flags {
		...
		if ((isLong || flag.Short == "") && strings.HasPrefix(long, inputStr)) ||
			...
			suggestion := editor.Suggestion[T]{
...
func (m Model[T]) allTokens(statement *statement) []editor.Token {
	tokens := []editor.Token{statement.Command.ToToken(0, "command")}
	for i, arg := range statement.Args.Value {
		tokens = append(tokens, arg.ToToken(i+1, "arg"))
	}
	for _, flag := range statement.Flags.Value {
		tokens = append(tokens, editor.TokenFromPos(flag.Name, "flag", len(tokens), flag.Pos))
		if flag.Value != nil {
...
func (m Model[T]) cursorInToken(tokens []editor.Token, pos int, roundingBehavior RoundingBehavior) bool {
	cursor := m.CursorIndex()
...
func (m Model[T]) currentTokenPos(tokens []editor.Token, roundingBehavior RoundingBehavior) TokenPos {
	cursor := m.CursorIndex()
	if len(tokens) > 0 {
...
func (m Model[T]) currentToken(tokens []editor.Token, roundingBehavior RoundingBehavior) []rune {
